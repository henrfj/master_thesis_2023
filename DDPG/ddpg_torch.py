...
class Agent(object):
    def __init__(self, alpha, beta, input_dims, tau, env, gamma=0.99,
                 ...
                 layer2_size=300, batch_size=64, chkpt_dir="checkpoints"):
        self.gamma = gamma
        self.tau = tau
...
class MPC_Agent(object):
    def __init__(self, alpha, beta, input_dims, tau, env, gamma=0.99,
                 n_actions=2, max_size=1000000, layer1_size=400,
                 layer2_size=300, batch_size=64, chkpt_dir="checkpoints"):
        self.gamma = gamma
        self.tau = tau
        # For storing all normal state, action, state, reward, terminal pairs
        self.memory = ReplayBuffer(max_size, input_dims, n_actions)
        # For storing halucinated knowledge; only connected to collision.
        self.halucinated_memory = ReplayBuffer(max_size, input_dims, n_actions)
        self.batch_size = batch_size
    ...
    def remember_halu(self, state, action, reward, new_state, done):
        # Remembering halucinated knowledge
        self.halucinated_memory.store_transition(state, action, reward, new_state, done)

    def learn(self, halu=False):
        if halu:
            if self.halucinated_memory.mem_cntr < self.batch_size:
                return
            state, action, reward, new_state, done = \
                                          self.halucinated_memory.sample_buffer(self.batch_size)
        else:
            if self.memory.mem_cntr < self.batch_size:
                return
            state, action, reward, new_state, done = \
