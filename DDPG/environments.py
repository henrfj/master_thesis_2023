...
class MPC_environment_v40(gym.Env): # 
	...
	- Instead of a single actions -> state -> reward, a **series of actions** will be simulated 
	"""
	metadata = {'render.modes': ['human']}

	def __init__(self, sim_dt=0.1, decision_dt=0.5, render=False, v_max=8, v_min=-2,
	       alpha_max=0.8, tau_steering=0.4, tau_throttle=0.4, horizon=200, edge=150,
		   episode_s=60, starting_point="", mpc=True):
		...
		self.edge = edge
		self.episode_seconds = episode_s
		# In case we are not starting from scratch
	...
	def reset(self):
		...
		#if self.mpc: # Will be used, for planning trajectories - to then be executed
		#	self.vision_box = Object(np.array([0, 0]), vertices=P2)
		#	self.trajectory = []
		#	
		#	# Two methods for updating vision
		#	self.update_vision_box_counter = 0
		#	self.SCs = [] # Would be used to compare and update, 
